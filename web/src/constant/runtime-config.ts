...
type RuntimeConfig = {
    ...
    SEEDANCE_ENABLED?: string; // Enable the server-managed Seedance channel
};
...
export const SEEDANCE_ENABLED = read("SEEDANCE_ENABLED", undefined) === "true";

export function seedanceBaseUrl() {
    if (typeof window === "undefined" || !window.location?.origin) return "";
    return `${window.location.origin}/sdk-api`;
}
