...
import { normalizeAiConfig, stripBuiltinSeedanceConfig, useConfigStore, type AiConfig, type WebdavSyncConfig } from "@/stores/use-config-store";
...
export function exportAppConfig() {
    ...
    const data: AppConfigFile = { app: "infinite-canvas", version: 1, exportedAt: new Date().toISOString(), config: stripBuiltinSeedanceConfig(config), webdav, promptSources: { sources, schedule } };
